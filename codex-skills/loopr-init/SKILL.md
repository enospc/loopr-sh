...
## Overview
Create or reuse a stable repo-id and set up transcript logging under specs/.loopr. Also scaffold decision log assets under specs/decisions/. This skill is idempotent and safe to run multiple times.
...
4. Ensure decision log scaffolding exists:
   - `specs/decisions/`
   - `specs/decisions/template.md` with headings: `Title`, `Date`, `Status`, `Context`, `Decision`, `Alternatives`, `Consequences`.
5. Define session log paths (do not overwrite):
...
6. Export environment variables for the current session (optional):
   - `LOOPR_REPO_ID=<repo-id>`
   - `LOOPR_TRANSCRIPT=<session-log-path>`
7. Note: transcripts are captured by the `loopr codex` wrapper. If you launch Codex directly, these files may remain empty.
8. If desired, append a short entry to `specs/.loopr/index.md` indicating the new session log path.
...
- Decision log template must live at `specs/decisions/template.md`.
